...
const createChart = async () => {
  ...
  try {
      ...
      if (workout.exercises) {
        workout.exercises.forEach((exercise: any) => {
          ...
          if (template) {
            ...
            muscleGroupCounts[primaryMuscle] = (muscleGroupCounts[primaryMuscle] || 0) + 1
            
            template.secondary_muscle_groups?.forEach((secondary: string) => {
              muscleGroupCounts[secondary] = (muscleGroupCounts[secondary] || 0) + 0.5
            })
          }
        })
      }
    ...
    const muscleGroupColors = [
      '#f47816', '#ff6b6b', '#4ecdc4', '#45b7d1', '#96ceb4',
    ...
    chart = new Chart(ctx, {
      type: 'doughnut',
      data: {
        labels: sortedMuscleGroups.map(([name]) => name.replace('_', ' ').toUpperCase()),
        datasets: [{
